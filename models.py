...
class RSSM(tools.Module):
    ...
    def observe(self, embed, action, state=None):
        ...
        post, prior = tools.static_scan(
            lambda prev, inputs: self.obs_step(prev[0], *inputs), (action, embed), (state_post, state_prior)
        )
    ...
    def obs_step(self, prev_state, prev_action, embed):
        ...
        stoch = (
            stoch_clean
            + tfd.MultivariateNormalDiag(tf.zeros_like(stoch_clean), tf.stop_gradient(prior["std"])).sample()
        )
        post = {
            "mean": tf.zeros_like(stoch),
            "std": tf.zeros_like(stoch),
            "stoch_clean": stoch_clean,
            "stoch": stoch,
            "deter": prior["deter"],
        }
        return post, prior
