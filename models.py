...
class RSSM(tools.Module):
  ...
  def initial(self, batch_size):
    ...
    return dict(
        ...
        stoch_clean=tf.zeros([batch_size, self._stoch_size], dtype),
        deter=self._cell.get_initial_state(None, batch_size, dtype))

  @tf.function
  def observe(self, embed, action, state=None):
    if state is None:
      state_post = self.initial(tf.shape(action)[0])
      state_prior = self.initial(tf.shape(action)[0])
    embed = tf.transpose(embed, [1, 0, 2])
    action = tf.transpose(action, [1, 0, 2])
    post, prior = tools.static_scan(
        lambda prev, inputs: self.obs_step(prev[0], *inputs),
        (action, embed), (state_post, state_prior))
  ...
  def obs_step(self, prev_state, prev_action, embed):
    ...
    stoch_clean = self.get('obs2', tfkl.Dense, self._stoch_size, None)(x)
    stoch = stoch_clean + tfd.MultivariateNormalDiag(
        tf.zeros_like(stoch_clean), tf.stop_gradient(prior['std'])).sample()
    post = {'mean': tf.zeros_like(stoch), 'std': tf.zeros_like(stoch),
            'stoch_clean': stoch_clean, 'stoch': stoch, 'deter': prior['deter']}
    return post, prior
  ...
  def img_step(self, prev_state, prev_action):
    ...
    prior = {'mean': mean, 'std': std, 'stoch': stoch, 'stoch_clean': stoch, 'deter': deter}
    return prior
